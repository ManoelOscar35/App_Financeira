...
import { DeleteDebts } from '../interface/deleteDebts';
import { DeleteRevenues } from '../interface/deleteRevenues';
import { DownloadImage } from '../interface/downloadImage';
import { ListDebts } from '../interface/listDebts';
import { ListRevenues } from '../interface/listRevenues';
import { LoginUser } from '../interface/loginUser';
import { RegisterDebts } from '../interface/registerDebts';
import { RegisterRevenues } from '../interface/registerRevenues';
import { RegisterUser } from '../interface/registerUser';
import { UpdateDebts } from '../interface/updateDebts';
import { UpdateRevenues } from '../interface/updateRevenues';
...
export class ApiService {
  ...
  getRegisterRevenues(param: any, user: any): Observable<ListRevenues> {
    ...
    return this.httpClient.get<ListRevenues>(`${environment.BASE_URL}/list/revenues`, {headers})
      .pipe(
        first(),
        catchError((err) => {
          if(err.status === 0 && err.status !== 404) {
  ...
  getRegisterDebts(param: any, user: any): Observable<ListDebts> {
    ...
    return this.httpClient.get<ListDebts>(`${environment.BASE_URL}/list/debts`, {headers})
      .pipe(
        first(),
        catchError((err) => {
          if(err.status === 0 && err.status !== 404) {
